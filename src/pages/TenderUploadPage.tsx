...
export default function TenderUploadPage() {
  ...
  const categoryColor: Record<CriterionCategory, string> = {
    Technical: 'bg-blue-50 dark:bg-blue-900/20 text-blue-700',
    Financial: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700',
    Compliance: 'bg-amber-50 dark:bg-amber-900/20 text-amber-700',
  };

  const weightColor: Record<CriterionWeight, string> = {
    Mandatory: 'bg-red-50 dark:bg-red-900/20 text-red-700',
    ...
    <div>
      ...
      <div className="p-8">
        ...
        {!uploaded && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Select or Create a Tender</h2>
              <button
            ...
            {tenders.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-50 mb-4">
                {tenders.map((t) => (
                  <button
                    key={t.id}
                    onClick={() => {
                    ...
                    className={`w-full px-5 py-3.5 text-left hover:bg-gray-50 dark:bg-gray-700/50 transition-colors flex items-center justify-between ${
                      t.id === selectedTenderId ? 'bg-navy-50' : ''
                    }`}
                  >
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t.title}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">{t.referenceNo}</p>
                    </div>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider ${
                      t.status === 'Completed' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700'
                        : t.status === 'Evaluating' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700'
                        : t.status === 'Parsed' ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700'
            ...
            {showNewTenderForm && (
              <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-4">Create New Tender</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Tender Title</label>
                    <input
                      type="text"
                      value={newTitle}
                  ...
                  <div>
                    <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Reference Number</label>
                    <input
                      type="text"
                      value={newRefNo}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <button
                      ...
                      className="px-4 py-2 text-xs font-medium text-gray-700 dark:text-gray-300 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:bg-gray-700/50 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
            ...
            {hasTender && (
              ...
              >
                <Upload className={`w-12 h-12 mx-auto mb-4 ${dragOver ? 'text-navy-600' : 'text-gray-400 dark:text-gray-500'}`} />
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Upload Tender Document</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-6">Drag and drop your tender document here, or click to browse</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-6">Supported formats: PDF, DOC, DOCX (max 50MB)</p>
                <button
                  onClick={() => {
        ...
        {isServerUploading && !isUploading && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Uploading tender file to secure storage...</span>
            </div>
          </div>
        )}
        ...
        {isUploading && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div className="flex items-center gap-3 mb-3">
              <Loader2 className="w-5 h-5 text-navy-600 animate-spin" />
              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Uploading document...</span>
            </div>
        ...
        {uploaded && !isUploading && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 mb-6 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center">
              <CheckCircle className="w-5 h-5 text-emerald-600" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{selectedFile?.name || 'Tender document'}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Uploaded successfully{selectedFile ? ` — ${(selectedFile.size / 1024 / 1024).toFixed(1)} MB` : ''}</p>
            </div>
        ...
        {uploaded && !isUploading && !parsed && (
          <div className="mb-6 flex items-center gap-3">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">OCR + criteria extraction will run only when you click this button.</span>
          </div>
        )}
        ...
        {isParsing && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div className="flex items-center gap-3 mb-3">
              <Loader2 className="w-5 h-5 text-navy-600 animate-spin" />
              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">AI is extracting eligibility criteria...</span>
            </div>
            <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-navy-600 rounded-full transition-all duration-200" style={{ width: `${parsingProgress}%` }} />
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-2">{parsingProgress}% — Analyzing document structure and requirements</p>
          </div>
        )}
        ...
        {parsed && !isParsing && tender && tender.criteria.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Extracted Eligibility Criteria</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">{tender.criteria.length} criteria identified — review and edit as needed</p>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-50 dark:bg-red-900/20 text-red-700">
            ...
            <div className="space-y-3">
              {tender.criteria.map((criterion) => (
                <div key={criterion.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 hover:shadow-sm transition-shadow">
                  {editingId === criterion.id ? (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Name</label>
                          <input
                            type="text"
                            value={editValues.name || ''}
                            onChange={(e) => setEditValues({ ...editValues, name: e.target.value })}
                            className="mt-1 w-full px-3 py-1.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-navy-500"
                          />
                        </div>
                        <div>
                          <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Threshold</label>
                          <input
                            type="text"
                      ...
                      <div>
                        <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Description</label>
                        <textarea
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Category</label>
                          <select
                        ...
                        <div>
                          <label className="text-[11px] font-medium text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Weight</label>
                          <select
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        ...
                        <button onClick={() => setEditingId(null)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:bg-gray-700/50 transition-colors">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">{criterion.name}</h3>
